...
func createVoters(cfg config) error {
	for ia, d := range cfg.topo.ASes {
		...
		// Sensitive voting cert (post-quantum)
		cmd = certs.Cmd(command.StringPather("certificate"))
		cmd.SetArgs([]string{
			"create",
			filepath.Join(votingDir, "PQ_sensitive.tmpl"),
			filepath.Join(votingDir, sensitiveCertNamePQ(ia)),
			filepath.Join(votingDir, "PQ_sensitive-voting.key"),
			"--profile=sensitive-voting",
			"--curve=ml-dsa",
			"--not-before=" + strconv.Itoa(int(cfg.now.Unix())),
			"--not-after=730d",
		...
		err = copyFile(
			filepath.Join(votingDir, "PQ_sensitive-voting.crt"),
			filepath.Join(votingDir, sensitiveCertNamePQ(ia)),
		)
		if err != nil {
			return err
		...
		// Regular voting (post-quantum)
		cmd = certs.Cmd(command.StringPather("certificate"))
		cmd.SetArgs([]string{
			"create",
			filepath.Join(votingDir, "PQ_regular.tmpl"),
			filepath.Join(votingDir, regularCertNamePQ(ia)),
			filepath.Join(votingDir, "PQ_regular-voting.key"),
			"--profile=regular-voting",
			"--curve=ml-dsa",
			"--not-before=" + strconv.Itoa(int(cfg.now.Unix())),
			"--not-after=730d",
		...
		err = copyFile(
			filepath.Join(votingDir, "PQ_regular-voting.crt"),
			filepath.Join(votingDir, regularCertNamePQ(ia)),
		)
		if err != nil {
			return err
...
func createCAs(cfg config) error {
	for ia, d := range cfg.topo.ASes {
		...
		fmt.Fprintf(cfg.writer, "Generate post-quantum CP Root and CP CA certificate for %s\n", ia)

		cmd = certs.Cmd(command.StringPather("certificate"))
		cmd.SetArgs([]string{
			"create",
			filepath.Join(caDir, "PQ_cp-root.tmpl"),
			filepath.Join(caDir, rootCertNamePQ(ia)),
			filepath.Join(caDir, "PQ_cp-root.key"),
			"--profile=cp-root",
			"--curve=ml-dsa",
			"--not-before=" + strconv.Itoa(int(cfg.now.Unix())),
			"--not-after=730d",
		...
		cmd.SetArgs([]string{
			"create",
			filepath.Join(caDir, "PQ_cp-ca.tmpl"),
			filepath.Join(caDir, caCertNamePQ(ia)),
			filepath.Join(caDir, "PQ_cp-ca.key"),
			"--profile=cp-ca",
			"--curve=ml-dsa",
			"--not-before=" + strconv.Itoa(int(cfg.now.Unix())),
			"--not-after=700d",
			"--ca=" + filepath.Join(caDir, rootCertNamePQ(ia)),
			"--ca-key=" + filepath.Join(caDir, "PQ_cp-root.key"),
		})
		if err := cmd.Execute(); err != nil {
			return err
		}
...
func createASes(cfg config) error {
	for ia, d := range cfg.topo.ASes {
		...
		fmt.Fprintf(cfg.writer, "Generate post-quantum CP AS certificate for %s issued by %s\n", ia, ca)

		cmd = certs.Cmd(command.StringPather("certificate"))
		cmd.SetArgs([]string{
			"create",
			filepath.Join(asDir, "PQ_cp-as.tmpl"),
			filepath.Join(asDir, chainNamePQ(ia)),
			filepath.Join(asDir, "PQ_cp-as.key"),
			"--profile=cp-as",
			"--curve=ml-dsa",
			"--not-before=" + strconv.Itoa(int(cfg.now.Unix())),
			"--not-after=" + util.FmtDuration(cfg.asValidity),
			"--ca=" + filepath.Join(caDir, caCertNamePQ(ca)),
			"--ca-key=" + filepath.Join(caDir, "PQ_cp-ca.key"),
			"--bundle",
		})
		if err := cmd.Execute(); err != nil {
			return err
	...
	for ia, d := range cfg.topo.ASes {
		...
		if d.Issuing {
			...
			certFiles[ia.ISD()] = append(certFiles[ia.ISD()],
				...
				filepath.Join(cryptoCADir(ia, cfg.out), rootCertNamePQ(ia)))
		}
		if d.Voting {
			...
			certFiles[ia.ISD()] = append(certFiles[ia.ISD()],
				...
				filepath.Join(cryptoVotingDir(ia, cfg.out), regularCertNamePQ(ia)),
				filepath.Join(cryptoVotingDir(ia, cfg.out), sensitiveCertNamePQ(ia)),
			)
		}
	}
	for isd := range isds {
		trcConf := conf.TRC{
			...
			VotingQuorumPQ:  uint8(len(voters[isd])/2 + 1),
		...
		parts := make(map[string]cppki.SignedTRC, len(voters[isd])*4)
		for _, voter := range voters[isd] {
			voterInfo, err := loadVoterInfo(voter, cryptoVotingDir(voter, cfg.out))
			if err != nil {
				return err
			}
			voterInfoPQ, err := loadVoterInfoPQ(voter, cryptoVotingDir(voter, cfg.out))
			if err != nil {
				return err
			}
			...
			sensitivePQ, err := signPayload(raw, voterInfoPQ.sensitiveKey, voterInfoPQ.sensitiveCert)
			if err != nil {
				return serrors.WrapStr("signing TRC payload - sensitive", err)
			}
			parts[fmt.Sprintf("ISD%d-B1-S1.%s-PQ_sensitive.trc", isd, voter)] = sensitivePQ
			regularPQ, err := signPayload(raw, voterInfoPQ.regularKey, voterInfoPQ.regularCert)
			if err != nil {
				return serrors.WrapStr("signing TRC payload - regular", err)
			}
			parts[fmt.Sprintf("ISD%d-B1-S1.%s-PQ_regular.trc", isd, voter)] = regularPQ
		}
...
func loadVoterInfoPQ(voter addr.IA, votingDir string) (*voterInfo, error) {
	sensitiveKey, err := key.LoadPrivateKey(
		filepath.Join(votingDir, "PQ_sensitive-voting.key"))
	if err != nil {
		return nil, serrors.WrapStr("loading sensitive key", err)
	}
	regularKey, err := key.LoadPrivateKey(filepath.Join(votingDir, "PQ_regular-voting.key"))
	if err != nil {
		return nil, serrors.WrapStr("loading regular key", err)
	}
	sensitiveCerts, err := cppki.ReadPEMCerts(
		filepath.Join(votingDir, sensitiveCertNamePQ(voter)))
	if err != nil {
		return nil, serrors.WrapStr("loading sensitive cert", err)
	}
	if len(sensitiveCerts) > 1 {
	...
	regularCerts, err := cppki.ReadPEMCerts(
		filepath.Join(votingDir, regularCertNamePQ(voter)))
	if err != nil {
		return nil, serrors.WrapStr("loading regular cert", err)
	}
	if len(regularCerts) > 1 {
...
func setupTemplates(cfg config) error {
	for ia, d := range cfg.topo.ASes {
		files := map[string]certs.SubjectVars{
			...
			filepath.Join(cryptoASDir(ia, cfg.out), "PQ_cp-as.tmpl"): {
				IA:         ia,
				CommonName: ia.String() + " PQ AS Certificate",
			},
		}
		if d.Issuing {
			...
			files[filepath.Join(cryptoCADir(ia, cfg.out), "PQ_cp-root.tmpl")] = certs.SubjectVars{
				IA:         ia,
				CommonName: ia.String() + " PQ Root Certificate - GEN I",
			}
			files[filepath.Join(cryptoCADir(ia, cfg.out), "PQ_cp-ca.tmpl")] = certs.SubjectVars{
				IA:         ia,
				CommonName: fmt.Sprintf("%s PQ CA Certificate - GEN I %d.1", ia, time.Now().Year()),
			}
		}
		if d.Voting {
			...
			files[filepath.Join(cryptoVotingDir(ia, cfg.out), "PQ_regular.tmpl")] = certs.SubjectVars{
				IA:         ia,
				CommonName: ia.String() + " PQ Regular Voting Certificate",
			}
			files[filepath.Join(cryptoVotingDir(ia, cfg.out), "PQ_sensitive.tmpl")] =
				certs.SubjectVars{
					IA:         ia,
					CommonName: ia.String() + " PQ Sensitive Voting Certificate",
				}
		}
...
func chainNamePQ(ia addr.IA) string {
	return fmt.Sprintf("%s_PQ.pem", fmtIA(ia))
}
...
func caCertNamePQ(ia addr.IA) string {
	return fmt.Sprintf("%s.PQ_ca.crt", fmtIA(ia))
}
...
func rootCertNamePQ(ia addr.IA, serial ...int) string {
	if len(serial) == 0 {
		return fmt.Sprintf("%s.PQ_root.crt", fmtIA(ia))
	}
	return fmt.Sprintf("%s.PQ_root.s%d.crt", fmtIA(ia), serial[0])
}
...
func sensitiveCertNamePQ(ia addr.IA, serial ...int) string {
	if len(serial) == 0 {
		return fmt.Sprintf("%s.PQ_sensitive.crt", fmtIA(ia))
	}
	return fmt.Sprintf("%s.PQ_sensitive.s%d.crt", fmtIA(ia), serial[0])
}
...
func regularCertNamePQ(ia addr.IA, serial ...int) string {
	if len(serial) == 0 {
		return fmt.Sprintf("%s.PQ_regular.crt", fmtIA(ia))
	}
	return fmt.Sprintf("%s.PQ_regular.s%d.crt", fmtIA(ia), serial[0])
}
