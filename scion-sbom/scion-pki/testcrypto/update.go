...
func extendTRC(now time.Time, out outConfig, predecessor cppki.SignedTRC) error {
	...
	pld := cppki.TRC{
		...
		QuorumPQ: 		   predecessor.TRC.QuorumPQ,
...
func regenTRC(now time.Time, out outConfig, predecessor cppki.SignedTRC) error {
	...
	for i, cert := range predecessor.TRC.Certificates {
		...
		case cppki.Sensitive:
			...
			case x509.MLDSA:
				newKey, err = createKeyPQ(fmt.Sprintf("%s/PQ_sensitive-voting.s%d.key", dir, id.Serial))
				if err != nil {
					return err
				}
			default:
		...
		case cppki.Regular:
			...
			switch cert.PublicKeyAlgorithm {
			case x509.MLDSA:
				newKey, err = createKeyPQ(fmt.Sprintf("%s/PQ_regular-voting.s%d.key", dir, id.Serial))
				if err != nil {
					return err
				}
			default:
		...
		case cppki.Root:
			...
			switch cert.PublicKeyAlgorithm {
			case x509.MLDSA:
				newKey, err = createKeyPQ(fmt.Sprintf("%s/PQ_cp-root.s%d.key", dir, id.Serial))
				if err != nil {
					return err
				}
			default:
...
func findkey(dir string, cert *x509.Certificate) (crypto.Signer, error) {
	...
	for _, file := range files {
		...
		case pq.PublicKey:
			certpkbytes := k.Bytes()
			pkbytes := key.(crypto.Signer).Public().(pq.PublicKey).Bytes()
			if bytes.Equal(pkbytes, certpkbytes){
...
func createKeyPQ(file string) (crypto.Signer, error) {
	key, err := pq.GeneratePrivKey()
	if err != nil {
		return nil, err
	}
	packed, err := x509.MarshalPKCS8PrivateKey(key)
