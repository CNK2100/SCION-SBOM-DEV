...
func (f *Fetcher) Level1(
	...
	// Prepare the request to use a QKD (if available).
	f.QKDStore.PrepareRequest(req)
	...
	for i := 0; i < f.MaxRetries; i++ {
		...
		if err == nil {
			...
			// If the request was using QKD, descramble the key.
			f.QKDStore.DescrambleKey(rep, &lvl1Key)
