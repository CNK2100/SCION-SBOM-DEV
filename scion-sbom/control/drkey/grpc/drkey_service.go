...
type Server struct {
	...
	QKDStore           qkd.Store // Quantum Key Distribution store if available, or nil.
	//                              Same QKD store as the one user by the fetcher.
}
...
func (d *Server) DRKeyLevel1(
	...
	// As a server, do the quantum scrambling part if available, noop otherwise.
	qkdKeyID, err := d.QKDStore.ScrambleKey(req, &lvl1Key.Key)
	resp := keyToLevel1Resp(lvl1Key)
	if err != nil {
		log.Error("Error scrambling the key in the QKD store", "err", err)
		return nil, serrors.WrapStr("Error scrambling the key in the QKD store", err)
	}
	resp.QkdKeyId = qkdKeyID
	resp.SaeId = d.QKDStore.MySaeId()
