...
import (
	...
	"github.com/scionproto/scion/pkg/drkey/qkd"
...
func realMain(ctx context.Context) error {
	...
	if globalCfg.DRKey.Enabled() {
		...
		// Create the Quantum Key Distribution store. If there is none available, the store
		// will be still created but with a noop (no operation) behavior.
		qkdConf := globalCfg.DRKey.QKD
		qkdStore, err := qkd.NewStoreFromCACert(
			qkdConf.URL,
			qkdConf.SaeID,
			qkdConf.CA,
			qkdConf.Cert,
		...
		if err != nil {
			return serrors.WrapStr("Unable to create the DRKey store", err)
		}
	...
	for ifid, intf := range topo.InterfaceInfoMap() {
		...
		// we can set normalLimit to anything reasonable; we decide what's reasonable or not
        
        var finalSbom uint64
        if sbomRaw > normalLimit {
            log.Error("SBOM underflow detected! Value rejected.", "ifid", ifid, "raw", sbomRaw)
            finalSbom = 0 // Reject and set to 0
        } else {
            finalSbom = sbomRaw
        }
