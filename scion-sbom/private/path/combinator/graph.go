...
func (solution *pathSolution) Path() Path {
	...
	// For the path to be PQ-secure: All AS-entries in all segments must have a PQ signature
	// We only check for the presence of such a signature here, verification happens elsewhere
	isPQsecure := true
	for _, s := range segments {
		for _, e := range s.ASEntries {
			...
			err := protobuf.Unmarshal(e.Pqsigned.HeaderAndBody, &hdrandbody)
			if err != nil {
				isPQsecure = false
				break
			}
			var header cryptopb.Header
			err = protobuf.Unmarshal(hdrandbody.Header, &header)
			if err != nil {
				isPQsecure = false
				break
			}

			if header.SignatureAlgorithm != cryptopb.SignatureAlgorithm_SIGNATURE_ALGORITHM_MLDSA44 {
				isPQsecure = false
				break
			}
		}
