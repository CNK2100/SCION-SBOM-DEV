...
func TestStaticinfo(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			checkSbom(t, g, tc.Path, metadata.Sbom)
...
func checkSbom(t *testing.T, g *graph.Graph,
	path []snet.PathInterface, Sbom []uint64) {
	...
	expected := []uint64{}
	for i := 0; i < len(path)-1; i++ {
		v := uint64(g.Sbom(uint16(path[i].ID), uint16(path[i+1].ID)))
		expected = append(expected, v)
	}
	assert.Equal(t, expected, Sbom)
}
