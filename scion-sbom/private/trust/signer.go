...
func (s Signer) SignPQ(
	ctx context.Context,
	msg []byte,
	associatedData ...[]byte,
) (*cryptopb.SignedMessage, error) {

	if s.PrivateKeyPQ == nil {
		return nil, nil
	}
	...
	var skid []byte
	switch k := s.PrivateKeyPQ.(type) {
	case pq.PQKeyPair:
		tmp := sha1.Sum(k.Pub.Bytes())
		skid = tmp[:]
	default:
		// This will trigger an error upon verification
		skid = []byte{}
	}
	id := &cppb.VerificationKeyID{
		IsdAs:        uint64(s.IA),
	...
	hdr := signed.Header{
		SignatureAlgorithm:   signed.MLDSA44,
		VerificationKeyID:    rawID,
		Timestamp:            now,
		AssociatedDataLength: associatedDataLen(associatedData...),
	}
	signedMsg, err := signed.Sign(hdr, msg, s.PrivateKeyPQ, associatedData...)
	if err != nil {
		metrics.Signer.Sign(l.WithResult(metrics.ErrInternal)).Inc()
		return nil, err
	}
