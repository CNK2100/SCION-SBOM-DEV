...
type Signer interface {
	...
	// XXX For multiple pq signatures make this return []SignedMessage
	SignPQ(ctx context.Context, msg []byte, associatedData ...[]byte) (*cryptopb.SignedMessage, error)
}
...
func (ps *PathSegment) AddASEntry(ctx context.Context, asEntry ASEntry, signer Signer) error {
	...
	signedMsg_raw, err := proto.Marshal(signedMsg)
	if err != nil {
		return serrors.WrapStr("packing signed message", err)
	}

	// FIXME inputs
	pq_sig , err := signer.SignPQ(ctx, []byte{}, signedMsg_raw)
	asEntry.Signed = signedMsg
	asEntry.Pqsigned = pq_sig
...
func (ps *PathSegment) VerifyASEntry(ctx context.Context, verifier Verifier, idx int) error {
	...
	_, err = verifier.Verify(ctx, ps.ASEntries[idx].Pqsigned, signedMsg_raw)
	return err
}
