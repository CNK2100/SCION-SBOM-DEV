...
func Verify(signed *cryptopb.SignedMessage, key crypto.PublicKey,
	...
	case pq.PublicKey:
		if !pq.Verify(pub, input, signed.Signature) {
			return nil, errors.New("mpi: msg: verification failure")
		}
...
func checkPubKeyAlgo(signAlgo SignatureAlgorithm, pubKey crypto.PublicKey) error {
	...
	case *ecdsa.PublicKey:
		...
		return nil
	case pq.PublicKey:
		if d.pubKeyAlgo != pkMLDSA44 {
			return errors.New("mpi: key algo vs signature algo")
		}
		return nil
	default:
