...
func (trc *TRC) Validate() error {
	...
	if trc.QuorumPQ < 0 || trc.QuorumPQ > 255 {
		return serrors.WithCtx(ErrInvalidQuorumSize, "voting_quorum_pq", trc.QuorumPQ)
	}
...
// Return wether the certificate is for a post-quantum key
func isPQ(cert *x509.Certificate) bool {
	// TODO better check for the actual supported alorithms
...
func (trc *TRC) ValidateUpdate(predecessor *TRC) (Update, error) {
	...
	// Count votes
	// - Non-pq votes are simply counted (every certificate may vote once)
	// - pq votes are accounted to an AS (an AS may vote multiple times with
	//   multiple certificates, but doing so counts as 1 vote)
	normalVotes := 0
	PQVotes := make(map[addr.IA]int)
	for _, i := range trc.Votes {
		if isPQ(predecessor.Certificates[i]) {
			ia, err := ExtractIA(predecessor.Certificates[i].Subject)
			if err != nil {
				return Update{}, serrors.WrapStr("extracting IA from certificate subject", err)
			}
			PQVotes[ia] += 1
		} else {
			normalVotes += 1
	...
	PQVotesAgg := len(PQVotes)

	if normalVotes < predecessor.Quorum {
		return Update{}, serrors.New("number of normal votes smaller than quorum",
			"quorum", predecessor.Quorum, "votes", normalVotes)
	}
	if PQVotesAgg < predecessor.QuorumPQ {
		return Update{}, serrors.New("number of PQ votes smaller than PQ quorum",
			"quorum", predecessor.QuorumPQ, "votes", PQVotesAgg)
	}
...
func (trc *TRC) validateRegular(predecessor *TRC, predCerts, thisCerts classified) (
	...
	if p_pq, n_pq := predecessor.QuorumPQ, trc.QuorumPQ; p_pq != n_pq {
		return nil, nil, serrors.New("quorum (PQ) changed", "predecessor", p_pq, "this", n_pq)
	}
