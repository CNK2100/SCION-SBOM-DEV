...
func (s Signer) SignPQ(ctx context.Context, msg []byte, associatedData ...[]byte) (*cryptopb.SignedMessage, error) {
	return nil, fmt.Errorf("SignPQ: unimplemented!")
}
...
func generateStaticInfo(g *Graph, ia addr.IA, inIF, outIF uint16) *staticinfo.Extension {
	...
	if outIF != 0 {
		Sbom.Intra = make(map[common.IFIDType]uint64)
		Sbom.Inter = make(map[common.IFIDType]uint64)
		for ifid := range as.IFIDs {
			if ifid != outIF {
				Sbom.Intra[common.IFIDType(ifid)] = g.Sbom(ifid, outIF)
			}
			if ifid == outIF || g.isPeer[ifid] {
				Sbom.Inter[common.IFIDType(ifid)] =
					g.Sbom(ifid, g.links[ifid])
			}
		}
	}
